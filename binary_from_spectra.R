...
# Import MALDIQuant picked peaks
shimadzu <- read.csv2('Shimadzu_csv_medianpeaks_df.csv', sep=',')
colnames(shimadzu) <- c('mass', 'intensity', 'spectrum')
shimadzu['TGNR']<-gsub('(\\d{6}(\\-.)*\\-\\d{2})(.*$)', '\\1\\2', shimadzu$spectrum)

# Bruker
Bruker <- read.csv2('Bruker_csv_medianpeaks_df.csv', sep = ',')
colnames(Bruker) <- c('mass', 'intensity', 'spectrum')
Bruker['code']<-gsub('([[:alnum:]]{8}\\-[[:alnum:]]{4}\\-[[:alnum:]]{4}\\-[[[:alnum:]]{4}\\-[[[:alnum:]]{12})(\\..*$)', '\\1', Bruker$spectrum)


# translate from bruker hash code to TGNR where necessary
trans <- read.csv2('Brukercode.tgnr.csv', header = T, sep = ',')
...
# import dereplicated strains (one strain per clinical case (n=825))
derep<-read.csv2('gff_paths.txt', header = F)
colnames(derep)<-'fullpath'
derep['gff'] <- gsub('(.*\\/)(.*)', '\\2', derep$fullpath)
derep['assembly'] <- gsub('.gff', '.fna', derep$gff)
...
colnames(phylo)<-c("sample","best_mash_dist", "Phylogroup", "Phylogroup_rough")
# only look at dereplicated strains
phylo['label'] <- phylo$sample
phylo <- phylo[phylo$label %in% derep$TGNR,]
phylo['TGNR']<-gsub('(\\d{6}(\\-.)*\\-\\d{2})(.*$)', '\\1\\2', phylo$label)
featureMatrix_binary<-merge(featureMatrix_binary, phylo[,c('label', 'Phylogroup')], by.y = 'label', by.x   = 'TGNR', all.x = T, all.y = F)

table(featureMatrix_binary[!duplicated(featureMatrix_binary$TGNR), 'Phylogroup'])
# remove unfrequent phylogroups
featureMatrix_binary <- featureMatrix_binary[!featureMatrix_binary$Phylogroup %in% c('E1','E2'),]
...
# find peaks which are present in at least 50% of the other group
...
# import PAPG VARIANT
pap<-read.delim('pagG_var.csv',quote="", sep=';',header=T, fill = TRUE)
#pap$X.FILE <- gsub("103713-19.fna", "103713-49.fna", pap$X.FILE)
pap$strain <- gsub("103713-19", "103713-49", pap$strain)
pap$strain <- gsub("712679-19", "712679-19-wh2", pap$strain)
pap <- pap[pap$strain %in% derep$TGNR,]
...
# find peaks which are present in at least 30% of the other group
...
pdf('../../../04_Presentations/occurence_plot_peaks_bruker_phylogroup.pdf', width = 10, height = 4)
occurence_plot_bruker
dev.off()

pdf('../../../04_Presentations/occurence_plot_peaks_shimadzu_phylogroup.pdf', width = 10, height = 4)
occurence_plot_shimadzu
dev.off()

pdf('../../../04_Presentations/occurence_plot_peaks_phylogroup.pdf', width = 20, height = 8)
...
pdf('../../../04_Presentations/MALDI_pap.pdf', height=3.5, width =9)
